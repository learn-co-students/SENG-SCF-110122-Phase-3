...
dogs = dogs_list.map{|attrs| Dog.create(attrs)}
...
walks = walks_list.map do |attrs| 
  Walk.create(attrs)
end

# we now have 3 walks persisted in our db, so we will run this process 3 times
Walk.ids.each do |walk_id|
  # variable that points to random number btw 1 and 5
  num_dogs = rand(1..5)
  # variable that creates an array with length of num_dogs and grabs random (unique) ids from Dog.ids
  dog_ids_array = Dog.ids.sample(num_dogs)
  # for each number in the dog_ids array, we create a DogWalk. So each Walk will have a DogWalk, num_dogs number of times (between 1 and 5 DogWalks for each Walk)
  dog_ids_array.each do |dog_id|
